...
export function hitungDenda(transaksi: any, logger: Logger): number {
  ...
  logger.log(`Perhitungan denda: ${fullDays} hari penuh (${hargaSewaPerHari}/hari) + ${extraHours} jam (${dendaPerJam}/jam) = ${totalDenda}`);
...
export function hitungTotalBiaya(
  ...
  logger.log(`Perhitungan biaya sewa: ${fullDays} hari (${hargaSewaPerHari}/hari) + ${extraHours} jam (${dendaPerJam}/jam) = ${totalBiaya}`);
